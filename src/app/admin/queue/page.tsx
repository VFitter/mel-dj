...
export default function AdminQueue() {
  ...
  function loadQueue() {
    fetch("/api/queue?includePlaying=1")
      .then((r) => r.json())
      .then(setQueue);
  }
  ...
  async function playNow(id: number) {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-white hidden lg:block">Queue</h1>
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <button onClick={skipCurrent} className="inline-flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-2.5 rounded-lg text-sm w-full sm:w-auto">
            <SkipForward className="h-4 w-4" />
            Skip Current
          </button>
          <button onClick={refill} className="inline-flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-2.5 rounded-lg text-sm w-full sm:w-auto">
        ...
        {queue.map((item, i) => (
          <div key={item.id} className={`rounded-lg border p-3 flex flex-col sm:flex-row sm:items-center gap-3 ${
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <span className="text-zinc-500 text-sm w-6 text-right shrink-0">{i + 1}</span>
              ...
              {item.type === "track" ? (
                <>
                  ...
                  />
                  <Music className="h-4 w-4 text-blue-400 flex-shrink-0 hidden sm:block" />
            ...
            <div className="flex items-center justify-between sm:justify-end gap-2 pl-9 sm:pl-0">
              ...
              {item.status === "pending" && (
                <div className="flex items-center gap-1">
                  <button onClick={() => playNow(item.id)} className="inline-flex items-center justify-center min-h-10 min-w-10 text-zinc-500 hover:text-blue-400 rounded-lg" title="Play now">
                    <Play className="h-4 w-4" />
                  </button>
                  <button onClick={() => removeItem(item.id)} className="inline-flex items-center justify-center min-h-10 min-w-10 text-zinc-500 hover:text-red-400 rounded-lg" title="Remove">
                    <SkipForward className="h-4 w-4" />
                  </button>
                </div>
              )}
